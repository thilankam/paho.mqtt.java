  ...
  // LoggerFactory.getLogger(LoggerFactory.MQTT_CLIENT_MSG_CAT,CLASS_NAME);
    ...
    // log.fine(CLASS_NAME, methodName, "600");
		...
		while (messageKeys.hasMoreElements()) {
          ...
          // log.fine(CLASS_NAME,methodName,"604", new Object[]{key,message});
              ...
              // log.fine(CLASS_NAME,methodName, "605", new
              // Object[]{key,message});
              ...
              // log.fine(CLASS_NAME,methodName, "606", new
              // Object[]{key,message});
						}
					} else {
            ...
            // log.fine(CLASS_NAME,methodName, "511", new
            // Object[]{key,message});
	...
	public void send(MqttWireMessage message, MqttToken token) throws MqttException {
		...
		if (message instanceof MqttPublish) {
          ...
          // log.fine(CLASS_NAME, methodName, "613", new Object[]{new
          // Integer(actualInFlight)});
        ...
        // log.fine(CLASS_NAME,methodName,"628", new Object[]{new
        // Integer(message.getMessageId()), new Integer(innerMessage.getQos()),
        // message});
      ...
      // log.fine(CLASS_NAME,methodName,"615", new Object[]{new
      // Integer(message.getMessageId()), message});
	...
	protected void undo(MqttPublish message) throws MqttPersistenceException {
      ...
      // log.fine(CLASS_NAME,methodName,"618", new Object[]{new
      // Integer(message.getMessageId()), new
      // Integer(message.getMessage().getQos())});
    ...
    // log.fine(CLASS_NAME,methodName,"616", new Object[]{});
		...
		if (connected && this.keepAlive > 0) {
          ...
          // log.severe(CLASS_NAME,methodName,"619", new Object[]{new
          // Long(this.keepAlive),new Long(lastOutboundActivity),new
          // Long(lastInboundActivity), new Long(time), new Long(lastPing)});
          ...
          // log.severe(CLASS_NAME,methodName,"642", new Object[]{new
          // Long(this.keepAlive),new Long(lastOutboundActivity),new
          // Long(lastInboundActivity), new Long(time), new Long(lastPing)});
          ...
          // log.fine(CLASS_NAME,methodName,"620", new Object[]{new
          // Long(this.keepAlive),new Long(lastOutboundActivity),new
      ...
      // log.fine(CLASS_NAME, methodName,"624", new Object[]{new
      // Long(nextPingTime)});
...
//				checkForActivity(); //Use pinger, don't check here
            ...
            // log.fine(CLASS_NAME,methodName,"617", new Object[]{new
            // Integer(inFlightPubRels)});
					}
            ...
            // log.fine(CLASS_NAME,methodName,"623",new Object[]{new
            // Integer(actualInFlight)});
					} else {
    ...
    // log.fine(CLASS_NAME,methodName,"625",new Object[]{message.getKey()});
        ...
        // log.fine(CLASS_NAME,methodName,"635",new Object[]{ new
        // Integer(pingOutstanding)});
            }
        }
	...
	private void decrementInFlight() {
      ...
      // log.fine(CLASS_NAME,methodName,"646",new Object[]{new
      // Integer(actualInFlight)});
...
//		if (quiescing && actualInFlight == 0 && pendingFlows.size() == 0 && inFlightPubRels == 0 && callback.isQuiesced()) {
      ...
      // log.fine(CLASS_NAME,methodName,"626",new Object[]{new
      // Boolean(quiescing), new Integer(actualInFlight), new
      // Integer(pendingFlows.size()), new Integer(inFlightPubRels),
    ...
    // new Integer(ack.getMessageId()), ack });
      ...
      // log.fine(CLASS_NAME,methodName,"636",new Object[]{ new
      // Integer(pingOutstanding)});
	...
	protected void notifyResult(MqttWireMessage ack, MqttToken token, MqttException ex) {
      ...
      // log.fine(CLASS_NAME,methodName, "648", new Object []
      // {token.internalTok.getKey(), ack, ex});
			callback.asyncOperationComplete(token);
		}
      ...
      // log.fine(CLASS_NAME,methodName, "649", new Object [] {
      // token.internalTok.getKey(), ex});
			callback.asyncOperationComplete(token);
		}
	}
	...
	public void quiesce(long timeout) {
      ...
      // log.fine(CLASS_NAME,methodName, "637",new Object[]{new Long(timeout)});
            ...
            // log.fine(CLASS_NAME, methodName,"639", new Object[]{new
            // Integer(actualInFlight), new Integer(pendingFlows.size()), new
            // Integer(inFlightPubRels), new Integer(tokc)});
    ...
    // log.fine(CLASS_NAME,methodName,"641", new Object[]{new
    // Integer(message.getMessageId())});
